...
import { Upload, FileText, Search, Trash2, MessageSquare, AlertCircle, X } from "lucide-react";
...
export default function Documents() {
  ...
  const getExt = (name: string) => name.split(".").pop()?.toUpperCase() ?? "DOC";
    ...
    <div className="flex-1 min-h-screen overflow-y-auto bg-[#0d0f1a] px-10 py-9">
      ...
      <div className="flex items-start justify-between mb-9 gap-4 flex-wrap">
        <div>
          ...
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-100">Documents</h1>
        ...
        <input hidden ref={fileInputRef} type="file" onChange={handleFileChange} multiple accept=".pdf,.doc,.docx,.txt" />
      </div>
      ...
      >
        {file ? (
          <div className="flex flex-col items-center">
            ...
            <h2 className="text-base font-semibold text-slate-200 mb-1">{file.name}</h2>
            <p className="text-sm text-slate-500 mb-5">{(file.size / 1024).toFixed(1)} KB · Ready to upload</p>
            <div className="flex gap-3 justify-center">
              <button
                onClick={(e) => { e.stopPropagation(); handleUploadDoc(); }}
          ...
          <div className="flex flex-col items-center">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 border transition-all duration-200
              ${isDragging ? "bg-indigo-500/25 border-indigo-500/40" : "bg-white/[0.04] border-white/[0.08]"}`}>
              <Upload size={22} className={isDragging ? "text-indigo-400" : "text-slate-500"} />
            </div>
            <h2 className="text-base font-semibold text-slate-400 mb-1">
              {isDragging ? "Drop to upload" : "Drag & drop your documents"}
            </h2>
            <p className="text-sm text-slate-600">
              or <span className="text-indigo-400 underline">browse files</span> · PDF, DOCX, TXT
            </p>
          </div>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] overflow-hidden">
        ...
        <div className="grid grid-cols-[1fr_100px_80px_60px] gap-4 px-5 py-3.5 border-b border-white/[0.08] bg-white/[0.02]">
          {["Document", "Size", "Chat", "Delete"].map((h) => (
            <div key={h} className="text-[11px] font-bold uppercase tracking-widest text-slate-600">{h}</div>
          ))}
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-3 min-w-0">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${extColor}`}>
                  <FileText size={16} className={iconColor} />
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-slate-200 truncate">{doc.doc_name}</p>
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${extColor}`}>{ext}</span>
                </div>
              </div>
              ...
              <button
                ...
                onClick={() => { setDocument({ doc_id: doc.doc_id, doc_name: doc.doc_name }); SetComponent("chats"); }}
              ...
              <button
                ...
                className="w-9 h-9 flex items-center justify-center rounded-lg bg-transparent border border-white/[0.08] text-slate-600 hover:bg-red-500/10 hover:border-red-500/30 hover:text-red-400 transition-all duration-150 cursor-pointer"
              >
                <Trash2 size={14} />
              </button>
            </div>
